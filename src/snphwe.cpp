// The function for exact HWE tests is copied from 
...
{
    int obs_geno = obs_hets + obs_hom1 + obs_hom2;
    double p1 = (obs_hets + 2.*obs_hom1)/(2.*obs_geno);
    double p2 = 1. - p1;
	
	if(p1 == 0 || p2 == 0) return 1;
    
    double exp_hets = 2*p1*p2*obs_geno;
    double exp_hom1 = p1*p1*obs_geno;
    double exp_hom2 = p2*p2*obs_geno;
    
    double chisq = pow(obs_hets-exp_hets, 2.)/exp_hets + pow(obs_hom1-exp_hom1, 2.)/exp_hom1 + pow(obs_hom2-exp_hom2, 2.)/exp_hom2;
    return getOneDfChisqPval(chisq);
}
